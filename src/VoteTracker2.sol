...
contract VoteTrackerDoubleYes {
    using CommonTypes for uint64;

    uint32 public voteStart;
    uint32 public voteLength;

    uint256 private yesVotesOption1;
    uint256 private yesVotesOption2;
    ...
    function castVote(uint256 vote) public voting(msg.sender) isRegistered(msg.sender) {
        ...
        if (vote_num == 0) {
            uint option = vote % 6;
            if (option > 3) {
                yesVotesOption2 += weight;
            } else {
                yesVotesOption1 += weight;
            }
        } else if (vote_num == 1) {
            noVotes += weight;
        } else {
    ...
    function getVoteResults() public view returns (uint256, uint256, uint256, uint256) {
        ...
        return (yesVotesOption1, yesVotesOption2, noVotes, abstainVotes);
    }
